...
object Dependencies {
  ...
  // HTTP servers. The keys are the template's `http_server` answers.
  val serverLibraries: Map[String, Seq[ModuleID]] = Map(
    "zio-http" -> Seq(
      "dev.zio" %% "zio-http" % V.zioHttp,
      // caliban-zio-http stopped at 2.x; caliban-quick (QuickAdapter) is its replacement.
      "com.github.ghostdogpr" %% "caliban-quick" % V.caliban
    ),
    "http4s" -> Seq(
